...
const withBluetoothCompileSdk = (config) => {
  return withProjectBuildGradle(config, (config) => {
    ...
    // This is the ONLY safe way to set compileSdk in Gradle 8.8
    subproject.beforeEvaluate { project ->
        // Register callback to configure android extension when plugin is applied
        // This works for plugins applied via 'apply plugin:' syntax
        project.plugins.whenPluginAdded { plugin ->
            if (plugin.class.name.contains('android.build.gradle') || 
                ...
                try {
                    ...
                    // Ignore if already set or too late
                }
            }
        }
        
        // Also try direct configuration if extension already exists
        try {
            ...
            // Extension doesn't exist yet - that's fine, whenPluginAdded will handle it
        }
    }
